...
async function callClaude(messages, system, maxTokens = 700) {
  ...
  // Anthropic/프록시 오류를 그대로 드러냄
  if (data && (data.type === "error" || data.error)) {
    const err = data.error || {};
    const msg = err.message || err.type || (typeof data.error === "string" ? data.error : "AI 응답 오류");
    throw new Error(msg);
  }
  const text = (data.content || [])
    ...
    .join("\n")
    .trim();
  if (!text) throw new Error("빈 응답을 받았어요. (모델 이름 또는 API 키 설정을 확인하세요)");
  return text;
}
...
function LumiChat({ drawing }) {
  ...
  const start = async () => {
    ...
    try {
      ...
      setMsgs([{ role: "assistant", content: out }]);
    } catch (e) {
      setMsgs([{ role: "assistant", content: "⚠️ 루미 연결 오류: " + e.message }]);
    }
    setBusy(false);
  };
  ...
  const send = async () => {
    ...
    try {
      ...
      setMsgs((s) => [...s, { role: "assistant", content: out }]);
    } catch (e) {
      setMsgs((s) => [...s, { role: "assistant", content: "⚠️ 루미 연결 오류: " + e.message }]);
    }
    setBusy(false);
  };
